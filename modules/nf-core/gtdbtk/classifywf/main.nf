process GTDBTK_CLASSIFYWF {
    ...
    path "gtdbtk.${meta.assembler}-${meta.id}.*.user_msa.fasta.gz"  , emit: user_msa
    ...
    gzip "classify/gtdbtk.${meta.assembler}-${meta.id}".*.classify.tree
    mv gtdbtk.log "gtdbtk.${meta.assembler}-${meta.id}.log"
    mv gtdbtk.warnings.log "gtdbtk.${meta.assembler}-${meta.id}.warnings.log"
    find . -type f \\( -iname \\*.gz -o -iname \\*.tsv -o -iname \\*.fasta \\) -exec ln -sf {} . \\;
